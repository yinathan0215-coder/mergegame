...
점수는 세 갈래로 오른다: **모든 행성–행성 충돌마다 +1**, **머지 시 생성 등급의 기본 점수**(등급이
높을수록 큰 점수), 그리고 **콤보(연속 머지)가 5단위에 도달할 때마다 주는 큰 보너스 점수**.
...
### 콤보 보너스 점수
- **콤보(연속 머지 체인, [[50-art-ux/feedback-effects]] §8)가 5의 배수에 도달할 때마다**(5·10·15·20·25…)
  **큰 보너스 점수**를 가산한다. 배율이 아니라 마일스톤마다의 **정액 보너스**(`콤보값 × bonusPer`)다 —
  배율 콤보([[40-balancing/decisions/2026-06-28-remove-combo]])와 달리 결정적·정밀하다. **콤보는 어렵게
  쌓이므로 보너스는 상당히 크게** 잡는다(값 SSoT `juice.combo`).
- 보너스 획득 시 **화면 중앙에 큰 `+N` 플로팅**(머지 +N보다 폰트 크고 유지시간 길다)으로 강조한다
