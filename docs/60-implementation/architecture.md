...
| 모듈 | 단일 책임 | 방법론 매핑 |
|---|---|---|
| `GameScene` | Pixi 앱·렌더 루프·씬 상태(`Title`↔`PoolInGame`)·시스템 오케스트레이션 | [[../90-methodology/state-machine]] · [[../90-methodology/game-loop]] |
...
| 모듈 | 단일 책임 | 방법론 매핑 |
|---|---|---|
| `TitleScreen` | Title 로비(태양계 공전 배경·최고/현재 점수·사이드 4버튼·Galaxy/Fantasy 토글) | [[../90-methodology/state-machine]] · [[../90-methodology/layered-rendering]] |
| `UnlockModal` | 신규 등급 첫 생성 시 해금 소개 모달(일시정지) | [[../90-methodology/state-machine]] · [[../90-methodology/layered-rendering]] |
| `MetaStore` | 메타 상태 권위: 코인 지갑·일일 미션·출석(KST 리셋·localStorage 영속) | [[../90-methodology/data-driven]] · [[../90-methodology/event-driven]] |
| `MetaUI` | 메타 팝업 라우터/레이어(일일미션·출석·룰렛·상점 열기) | [[../90-methodology/layered-rendering]] · [[../90-methodology/event-driven]] |
| `popups/` (`DailyMission`·`Attendance`·`LuckyWheel`·`Shop`) | 각 메타 기능 팝업 뷰 | [[../90-methodology/layered-rendering]] |
| `ui/` (`Popup`·`button`·`coin`) | 공통 UI 프리미티브(팝업 틀·버튼 프레스 피드백·코인 칩) | [[../90-methodology/layered-rendering]] |
...
- `balance.json` — 단일 출처: `planets`(11단계 반지름·점수·색·패턴) · `scoring` · `juice`(`combo` 포함) ·
  `queue` · `rack` · `launch` · `physics` · `progression` · `layout`(HUD/보드/PLAY/발사대) · `colors` ·
  `engine` · `economy` · `dailyMissions` · `attendance` · `wheel` · `sound`.
