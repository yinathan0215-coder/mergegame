...
- **모든 전체 화면 팝업**(메타 팝업 일일 미션·출석·돌림판·상점·설정 + **인게임 해금 모달**)은 패널·행성·
  버튼이 9:16 밖으로 잘리거나 과대해지지 않도록 **contain 레이어(`fgRoot`)**에 둔다(`raw: game/src/ui/Popup.ts`
  · `game/src/UnlockModal.ts`). 콘텐츠는 contain 스케일로 **모든 화면비(웹 16:9 포함)에서 정상 크기**로 보이고,
  **딤만 DESIGN 영역을 크게 넘는 사각**으로 그려 contain 변환 아래에서도 레터박스까지 화면 전체를 덮는다.
