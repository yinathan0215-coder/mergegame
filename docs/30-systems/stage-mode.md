...
- **목표 행성이 합성되는 순간** 추가 발사가 **즉시 정지**되고 **발사대(챔버)의 대기 행성도 비워진다**
  (이후 입력으로 더 못 쏜다). 보드 물리도 그 순간 멈춘다.
- **클리어 연출:** 방금 만들어진 목표 행성이 **우하단 목표 UI의 행성으로 포물선을 그리며 날아가**
  합쳐지는 이펙트(머지 버스트)를 내고 사라진다 — **실제로 합성되지는 않고**(보드에서 제거된 뒤 날아가는
  연출 스프라이트), 목표 UI에 도달하면 사라진다([[50-art-ux/result-window]] §Stage Clear). 연출이
  끝나면 클리어 결과창이 전환 효과와 함께 뜬다(보상 **코인 +300**, [[30-systems/meta-economy]] 지갑) →
  하단 **[다음 스테이지]** / **[돌아가기]**(→Title).
- **클리어 = 다음 스테이지로 진행(영속).** 스테이지를 클리어하면 **현재 스테이지 포인터가 다음
  스테이지로 전진**하고 `localStorage`에 저장된다. **[다음 스테이지]/[돌아가기] 어느 쪽을 눌러도** 이후
  Title의 `Stage N`과 다음 플레이 진입 스테이지는 **다음 스테이지**다(예: Stage 1 클리어 → 이후 Stage 2).
  마지막 정의 스테이지에서는 더 전진하지 않는다.
- **이미 클리어한 스테이지는 다시 클리어되지 않는다.** 클리어한 스테이지 인덱스는 영속 저장되며, 목표를
  다시 만들어도 보상·클리어창이 발생하지 않는다(중복 보상 방지). 정상 플레이는 항상 가장 앞선 미클리어
  스테이지로 진입하므로 클리어한 스테이지를 다시 플레이하지 않는다.

### 단계 해금 모달 없음 (Stage)
- **Stage는 합성 단계 해금 모달을 띄우지 않는다** — 해금 연출은 Infinite 전용이다([[30-systems/tier-unlock]]).
  Stage는 시작부터 **모든 단계가 해금된 상태**(`unlockedTier` = 최대 단계)라 목표까지 합성이 막히지 않고
  중간 단계에서 모달/일시정지가 끼어들지 않는다.
