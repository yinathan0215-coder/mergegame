...
### 종료 판정
- **Infinite:** 카운트 0 **그리고** 보드의 모든 행성이 정지(속도 임계 이하)하면 세션이 끝나고
  결과창이 뜬다([[50-art-ux/result-window]]). 마지막 발사의 합성 연쇄가 끝날 때까지 기다린다.
- **Stage:** 카운트 0 그리고 목표 미달 + 모든 행성 정지 → 실패([[30-systems/stage-mode]]).
  목표 행성이 만들어지면 카운트가 남아 있어도 즉시 클리어다.
