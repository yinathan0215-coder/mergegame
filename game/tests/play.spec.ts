...
test('첫 해금: 잠긴 등급(천왕성) 머지 → 해금 팝업·일시정지 → OK로 해금', async ({ page }) => {
  await ready(page);
  await page.evaluate(() => window.__game.clearBoard()); // 랙 격리
  expect(await page.evaluate(() => window.__game.unlockedTier())).toBe(5); // 기본 해금: 해왕성(5)까지
  // 해왕성(5) 2개 → 천왕성(6): 아직 잠긴 등급 → 해금 팝업 + 일시정지
  await page.evaluate(() => window.__game.spawnPair(5));
