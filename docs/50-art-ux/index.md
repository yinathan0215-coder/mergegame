...
- [[title-screen]] — **Title(로비) 화면**: 태양계 공전 배경(Pool 리소스 재사용) + 로비 UI(최고/현재 점수·사이드 4버튼·Infinite/Stage 모드 토글·설정) + 버튼 피드백.
- [[title-icons]] — **Title 아이콘 리소스**: 설정·일일 미션·출석 체크·행운의 돌림판·상점 PNG와 `$imagegen` 프롬프트 정본. 실사 금지, 기존 왕관/머니 계열 캐주얼 2.5D 스타일을 따른다.
- [[button-system]] — **공통 버튼 규칙**: 이미지 아이콘 버튼의 검은 반투명 배경, 라벨 분리, Play 9-slice, 공통 눌림 피드백.
- [[galaxy-background]] — **공유 Galaxy background**: 정적 캐주얼 우주 물결 배경 + 별·점광 런타임 루핑 반짝임. Pool In-Game 보드 내부와 Title 배경이 같은 시스템을 쓴다.
- [[layout]] — 화면 비율(9:16) + **상단 HUD 배치**(좌: 나가기 / 중앙: Score·최고점수 / 우: 메뉴) + 제외 UI.
...
- [[art-direction]] — §9 전체 톤(네이비/골드/우주/은하/별, 장식 식별 방해 금지) + 행성 아이콘 규칙 + §13 아트 판단 4개.
...
- [[result-window]] — **결과창**: Infinite 결과(스코어 1→최종 카운트업·NEW RECORD·최대 콤보)·Stage 클리어/실패, 공통 팝업 전환.
