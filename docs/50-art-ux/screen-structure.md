...
- 모서리·테이퍼 연결부 반경: outline의 `cornerR`(위 모서리)·`junctionR`(테이퍼 연결)을 **inset보다 충분히
  크게** 잡아 inner line(=offset)도 둥글게 유지한다.
...
- 보드 내부 galaxy background는 [[galaxy-background]]를 따른다. 정적 이미지는 은하수/성운 흐름만
  담당하고, 작은 별·점광·반짝임은 같은 clip 영역 안에서 루핑 효과로 렌더한다.
