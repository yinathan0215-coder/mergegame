...
**91.0/100**(7차원 만점). GameScene 810→570(6모듈 분리), 13 수정 커밋, Playwright 80/0, 회귀 0.
**종료 사유**: 남은 2건 중 D1=3은 사용자 balance.json 미커밋 WIP 차단(해소 시 진행), D2=4는 감사자 판정상
**5 추구가 과분할**(세션종료 흐름=phase 상태머신 종료 전이 → 분리 시 D4=5 훼손) → 수정 안 함이 옳음.
사용자 WIP(balance.json·ModeController) 미수정 유지.
...
docs/90-methodology/(프로젝트 부록)에 단일 출처로 배치. 각 GDD 섹션(10–70)에 "준수 기준" 바인딩,
CLAUDE.md에 인덱스 링크 연결. See [[90-methodology/index]].

## [2026-06-27] manual | 프로젝트 문서 정렬로 docs/ 구조 재설계
why: HTML5 Merge 프로토타입용 에이전트 실행 기획문서 구조에 맞춰
...
## [2026-06-29] manual | Remove external review framing
why: Keep this repo focused on the game itself by replacing external evaluation labels with
project/spec/release terminology across docs.
