...
- **등장 전환:** 모달은 **딤이 차오르고 내용(행성·OK 버튼)이 살짝 커지며 페이드인**하는 짧은 전환으로
  등장한다(즉시 튀어나오지 않는다).
- **딤드가 화면 전체를 덮는다:** 보드(캔버스)뿐 아니라 캔버스 바깥 **레터박스(좌·우·상·하 여백)까지**
  빈틈 없이 어둡게 한다. 캔버스는 9:16 contain-fit이라 Pixi 딤은 보드만 덮으므로, 레터박스는 캔버스
  뒤 DOM 딤으로 덮어 화면 전체가 하나의 연속된 딤으로 보이게 한다.
- **화면 중앙에 해금된 행성**(단계 T 스프라이트)이 **천천히 회전**하고, 그 **위에 행성 영어 이름**
  (예: Neptune·Uranus, SSoT `balance.json` `planets[].en`)이 **똑바로(회전하지 않음)** 표시된다.
