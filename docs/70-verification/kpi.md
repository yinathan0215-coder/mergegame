...
`game/` 프로토타입 구현 완료. **Playwright 실플레이 16/16 통과**(desktop 1280×800 + mobile
390×844) — 초기 랙 10, 발사 후 발사대에 새 행성 로드, 동급 충돌 100%
합성 + 충돌 방향 이동, 태양 종단, 첫 5발 내 ≥1 합성, 벽 반사(보드 누출 0), **일방향 경계**(진입
후 선 아래 복귀 0), 실드래그 발사. **방패형 보드**(직사각형+테이퍼+둥근 볼록 끝)·9:16 유지·DOM
컨트롤 0(포켓/Shake/Change Ball 없음). 프로덕션 빌드 OK. 스펙: `game/tests/play.spec.ts`,
