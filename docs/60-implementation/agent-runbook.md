...
- 포켓 / 포켓 관련 UI
- Shake 버튼 · Change Ball 버튼 · 광고 보상 실연동(더미 미션만)
- 상단 행성 진행 트랙 · 미도달 단계 실루엣

**프로토타입 필수 범위 아님:**

- 랭킹(리더보드) · 튜토리얼 · 조준 중 취소 기능

> **구현 범위에 포함**(과거 비범위였으나 현재 빌드 반영): 사운드 · `localStorage` 저장 ·
> 게임 모드(Infinite/Stage) · 세션 종료(결과·스테이지 클리어/실패 창). 클래식 "게임 오버(사망/리트라이)"는
> 없고, 종료는 **카운트 소진 결과창**이 대신한다([[../20-core-loop/game-modes]]).
