...
> 등장 전환·딤). 레퍼런스(캐주얼 머지 게임 설정창) 구색을 맞추되, **실제 동작은 `사운드`(마스터 뮤트)와
> 하단 `게임 초기화`뿐**이고 나머지는 비동작 placeholder다.
...
| 하단 | **게임 초기화**(빨강) | **동작** — 로컬 저장 전체 삭제 후 첫 로딩 화면으로 재시작 |
...
- 하단 **`게임 초기화`** 버튼(빨강 경고색)은 **로컬 저장소를 전부 비우고**(`localStorage.clear()` — 코인
  지갑·미션·출석·점수 레코드·사운드 설정 등 모든 키) **첫 로딩 화면(GALAXY PINBALL 스플래시,
  [[../20-core-loop/screen-flow]] §Loading)으로 재시작**한다. 저장을 비운 뒤 페이지를 새로 로드해 부팅을
  처음부터 다시 태운다 — 코인·진행도·기록이 모두 0으로 돌아간다.
