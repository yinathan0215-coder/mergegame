...
- **merge lock:** 같은 물리 tick에서 하나의 행성이 둘 이상의 합성에 동시에 사용되지 않도록
  잠근다(한 tick·한 행성 1합성).
- **재합성 지연(머지 결과 전용):** **합성으로 생성된** 새 행성만 짧은 시간 추가 합성 판정을 지연시켜
  겹침에 의한 즉시 재합성을 막는다. **발사·초기 랙 행성은 지연 없이 첫 충돌에서 바로 합성**된다.
  지연 시간 값은 [[40-balancing/index]].
- **연쇄 허용:** 위 지연을 둔 채로도 **자연스러운 연쇄 합성은 허용**한다 — 연쇄가 쌓이면 **콤보
  보너스**로 이어진다(점수 규칙은 [[30-systems/scoring-combo]]).
