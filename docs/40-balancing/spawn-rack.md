...
- **`progression`(수치 SSoT):** `unlockStart` = **5**, `queueBelow` = **2**, `queueCap` = **5**(해왕성).
- **시작 시:** `unlockedTier`=5 → 후보 = **수성·화성·금성 3종**(균등 1/3). 해금이 오르면 후보가
  늘되 **해왕성(5)을 넘지 않는다**(천왕성↑은 큐에 안 들어오고 합성으로만 등장).
...
- **배치 = 역삼각형(▽):** 위가 넓고(수성 4개) 아래로 갈수록 좁아져(지구 1개) 아래를 향하는 삼각형.
