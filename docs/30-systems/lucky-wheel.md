...
- 룰렛이 멈추면 결과 코인은 **쏟아짐 연출을 통해 적립**된다 — 룰렛 중심에서 **코인 스프라이트가
  쏟아져** 왼쪽 상단 **코인 표시 위치로 날아간다**.
- 쏟아지는 스프라이트 수 = **결과값 ÷ 10**(`wheel.coinsPerPourSprite` = 10). 예: 결과 250 → 25개,
  10 → 1개.
- 코인이 날아오는 동안 **코인 표시의 수치가 1씩 굴러 오르며**(롤링 카운트업) 최종 잔액
  (**이전 + 결과값**)까지 증가한다 — 즉 지급량은 결과값 그대로다.
