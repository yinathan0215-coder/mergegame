...
- **해금 보너스:** 새 단계 해금 팝업(넵튠 6단계부터)이 뜰 때마다 **카운트 +5** 를 준다(Infinite
  한정). 해금 팝업의 OK 버튼 위에 `Count +5` 가 강조 표시된다([[../30-systems/tier-unlock]]).
- **종료 조건:** **카운트를 모두 소진하면 2초 뒤** 세션이 끝나고 결과창이 뜬다(마지막 발사·합성
  연출의 여운을 위한 지연). 결과창은 전환 효과와 함께 등장해 스코어가 1부터 최종 점수까지
...
- **클리어 조건:** 카운트 안에 **목표 행성을 합성으로 만들면** 성공이다. 달성 **2초 뒤** 클리어
  결과창이 전환 효과와 함께 뜨고 **코인 +300** 을 준다. 결과창 하단에는 **[다음 스테이지]** 와
  **[돌아가기]**(→Title) 버튼이 있다.
...
| 위치 | Infinite | Stage |
|---|---|---|
| **하단 좌측** | 남은 Count + 큐의 Next 미리보기(가로 나란히) | 남은 Count + 큐의 Next(가로 나란히) |
...
| From | Event | To | 조건 |
|---|---|---|---|
| `PoolInGame(Infinite)` | `COUNT_DEPLETED` → 2초 | `Result` | 결과창(전환·카운트업) |
