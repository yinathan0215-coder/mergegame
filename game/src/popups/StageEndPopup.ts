...
function bigButton(cx: number, cy: number, label: string, base: number, onTap: () => void): Container {
  ...
  const t = new Text(label, { fill: COLORS.white, fontSize: TYPE.s20, fontFamily: FONT, fontWeight: '800' });
  t.anchor.set(0.5);
  t.y = BUTTON3D_DY;
  c.addChild(t);
...
export class StageClearPopup extends Popup {
  constructor(onNext: () => void, onHome: () => void) {
    ...
    const cx = DESIGN.w / 2;
    this.body.addChild(centred('스테이지 클리어!', 24, COLORS.white, 300));
    const coin = coinSprite(34);
    coin.x = cx - 40;
    coin.y = 372;
    const reward = new Text(`+${MODES.stage.clearReward}`, { fill: COLORS.gold, fontSize: TYPE.s34, fontFamily: FONT, fontWeight: '800' });
    ...
    this.body.addChild(bigButton(cx, 596, '돌아가기', COLORS.settingsGrey, () => { this.close(); onHome(); }));
  }
}
...
export class StageFailPopup extends Popup {
  constructor(onRetry: () => void) {
    ...
    const cx = DESIGN.w / 2;
    this.body.addChild(centred('목표 행성을 만들지 못했어요', 20, COLORS.textBlue, 320));
