...
점수는 세 갈래로 오른다: **충돌 점수(벽 +1, 행성–행성 +3)**, **머지 시 생성 등급의 기본 점수**(등급이
...
- **콤보(연속 머지 체인, [[50-art-ux/feedback-effects]] §8)가 5의 배수에 도달할 때마다**(5·10·15·20·25…)
  **큰 보너스 점수**를 가산한다. 마일스톤마다의 **정액 보너스 = `콤보값 × bonusPer`**(예: 5콤보 → 5×400 =
  2000점)로 결정적이다. **콤보는 어렵게 쌓이므로 보너스는 상당히 크게** 잡는다(값 SSoT `juice.combo`의
  `step`·`bonusPer`).
