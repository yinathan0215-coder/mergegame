...
> 문서 범위 ② — Pool In-Game은 **두 게임 모드**로 플레이된다: **Infinite(무한)** 와
...
- **클리어 조건:** 카운트 안에 **목표 행성을 합성으로 만들면** 성공이다. 목표 합성 순간 **추가 발사
  정지·발사대 비움**, 목표 행성이 **우하단 목표 UI로 포물선 비행 → 합쳐지는 연출**(실제 합성 아님) 후
  클리어 결과창이 뜨고 **코인 +300** 을 준다([[../50-art-ux/result-window]] §Stage Clear). 결과창
  하단에는 **[다음 스테이지]** 와 **[돌아가기]**(→Title) 버튼이 있다.
- **클리어 = 다음 스테이지로 진행(영속).** 스테이지를 클리어하면 현재 스테이지 포인터가 다음으로 전진해
  `localStorage`에 저장된다 — [다음 스테이지]/[돌아가기] 어느 쪽이든 이후 Title `Stage N`·다음 진입은
  다음 스테이지다(Stage 1 클리어 → 이후 Stage 2).
- **해금 모달 없음:** Stage는 합성 단계 해금 모달을 띄우지 않는다(시작부터 전 단계 해금, 해금 연출은
  Infinite 전용, [[../30-systems/tier-unlock]]).
- **이미 클리어한 스테이지는 다시 클리어되지 않는다** — 클리어 기록이 영속되며, 목표를 다시 만들어도
  보상·클리어창이 뜨지 않는다(중복 보상 방지, [[../30-systems/stage-mode]]).
...
| `PoolInGame(Stage)` | 목표 행성 생성 → 비행+버스트 연출 완료 | `StageClear` | +300코인·전환·스테이지 포인터 전진(영속) |
| `PoolInGame(Stage)` | `COUNT_DEPLETED` & 목표 미달 → 2초 | `StageFail` | Fail 결과창(전환) |
| `Result` | tap | `Title` | — |
| `StageClear` | [다음 스테이지] / [돌아가기] | 다음 스테이지 / `Title` | (클리어 시점에 이미 다음으로 전진) |
