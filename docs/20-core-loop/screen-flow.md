...
Pool 세션 상태·점수는 **`localStorage`에 저장**되어 Title의 "현재 점수"로 다시 보이며,
새로고침 후에도 이어진다(점수 영속 정본 [[../50-art-ux/title-screen]] §2-2).
...
- **표시:** 와인색 `background color` 위에 **게임 로고(`PLANET POOL MERGE` 텍스트) + 하단 프로그레스 바**.
