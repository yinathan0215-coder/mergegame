...
## Open questions
- 본 게임의 발사 방향을 **측면(좌↔우)**으로 할지 **상방(아래에서 위로 쏘기)**으로 할지 — 미정.
- "Tasty Travels launch 모드"가 실제 본편 내 이벤트/미니게임으로 존재하는지(광고 전용이 아니라)는
  본편 장기 플레이 영상으로만 최종 확인 가능 — 현재는 *광고 미니게임 가설*이 가장 유력. 🟡
- 위 🟡 항목(다운로드 수, 진화 체인 세부, 대조군 클론 메커닉)은 검증 단계 rate-limit으로
  미독립검증 — 컨셉 확정 전 재확인 권장.
