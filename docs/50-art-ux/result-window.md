...
### Stage Clear
- **등장:** 전환 효과와 함께 클리어 결과창이 뜬다(보상 **코인 +300** 지급 후).
- **버튼(하단 2개):** **[다음 스테이지]**(다음 레벨 보드로) · **[돌아가기]**(→Title).
  ([[../30-systems/stage-mode]]).

### Stage Fail
- **등장:** 전환 효과와 함께 Fail 결과창이 뜬다.
- **나가기:** 닫으면 **게임 화면(같은 스테이지 보드)으로 복귀**한다.

### 공통
- 결과창은 **cover 팝업 레이어**(`popupRoot`)에 그려져 뷰포트 전체 딤 위에 중앙 배치된다
  ([[popup-system]] §입력·딤). 동시에 하나만 뜬다.
