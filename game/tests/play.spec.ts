...
test('일방향 경계: 진입 후 행성이 분리 선 아래로 복귀하지 않는다', async ({ page }) => {
  await ready(page);
  await page.evaluate(() => window.__game.clearBoard());
  const lineY = await page.evaluate(() => window.__game.lineY());
  // 발사: 위로(살짝 비스듬) — 선을 넘어 플레이 영역 진입
  ...
  // 이후 2초간 inPlayArea 행성이 선 아래(+8px 여유)로 내려오지 않음
  let returned = false;
  for (let i = 0; i < 12; i++) {
    const bad = await page.evaluate(
      (ly) => window.__game.snapshot().some((p) => p.inPlayArea && p.y > ly + 8),
      lineY
    );
    if (bad) {
      returned = true;
...
test('절대 영역: 최대 파워 발사·강한 충돌에도 행성이 플레이 영역을 벗어나지 않는다', async ({ page }) => {
  ...
  // 일방향: 진입한 행성은 분리 선 아래로 내려가지 않는다(벽처럼 막힘)
