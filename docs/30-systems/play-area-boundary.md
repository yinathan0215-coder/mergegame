...
보드의 **충돌 경계는 inner line**(방패형 보드 윤곽: 둥근 직사각형 + 140° 테이퍼 + **발사대 원 윗호**)과
**발사대 원형 공간**이다. inner line은 outline을 안쪽으로 균일 오프셋한 갈색 선이고, 그 하단은 발사대 원
**윗호로 닫혀 발사대 원은 inner line 바깥**(아래)에 놓인다. 행성은 inner line과 발사대 원에 부딪혀 보드
안에 머문다.
...
- **발사 일방향(발사대 원 예외):** 갓 발사된 행성은 발사대 원과 충돌하지 않고 **밖으로 빠져나간다**.
  ...
  **발사대 원에만** 적용한다.
- **절대 영역(Absolute containment).** 행성은 **어떤 속도에서도** inner line 안에 머문다. 충돌 벽을
  **공의 한 스텝 이동량(최대 발사 속도)보다 두껍게** 만들어, 강한 충돌·발사로도 관통하지 못하게 한다.

## 절대 영역 강제 (두꺼운 벽 + clamp backstop)

**1차(정본):** inner line·테이퍼·하단 호·발사대 원을 **두꺼운 정적 충돌 벽**으로 만든다. 벽 두께가
공의 한 스텝 이동량(≈ 최대 발사 속도)보다 크면 Matter에 연속 충돌 검출(CCD)이 없어도 관통이 불가능하다.
벽은 **안쪽 면을 윤곽에 맞추고 바깥으로 오프셋**해, 보이는 경계는 그대로 두고 두께만 늘린다.

**2차(backstop):** 사각 플레이 영역 clamp가 매 고정 서브스텝마다 위치를 보정하고 경계 방향 속도를
반사(× `wallRestitution`)해 극단 케이스를 잡는다.

```
사각 backstop, 행성마다 (r=반지름, e=wallRestitution):
...
- inner line·테이퍼·하단 호·발사대 원을 **두꺼운** 정적 충돌 바디로 만든다(두께 ≥ 최대 발사 속도,
  안쪽 면은 윤곽에 맞추고 바깥으로 오프셋). **분리 선 벽은 두지 않는다.**
...
- 사각 영역 clamp-and-reflect는 보조 backstop이다(두꺼운 충돌 벽이 1차 정본). 수치는 `balance.json`.
