...
| `rack` | **시작 랙 구성**: `{ tier, count }` 목록(등급별 개수). 코드가 등급당 한 줄씩 **모양**으로 배치한다(밸런싱 [[40-balancing/stage-balance]]) |
| `queue` | **발사 큐 시퀀스**: 등장 순서대로의 `tier` 배열(결정적, 지구/5 이하). 길이가 카운트보다 짧으면 마지막 규칙으로 보충 |

- Infinite의 랜덤 큐·기본 랙([[30-systems/launch-queue]] · [[30-systems/initial-rack]])과 달리,
  Stage는 **랙과 큐를 스테이지가 직접 지정**한다(결정적 레벨 디자인).

### 클리어
- **카운트 안에 `target` 등급 행성을 합성으로 만들면 클리어.** (카운트가 남아 있어도 됨)
- 달성 **2초 뒤** 클리어 결과창이 뜬다(대기 중 **화면을 탭하면 즉시**). 보상 **코인 +300**
  ([[30-systems/meta-economy]] 지갑) → 하단 **[다음 스테이지]** / **[돌아가기]**(→Title)
  ([[50-art-ux/result-window]]).
- **이미 클리어한 스테이지는 다시 클리어되지 않는다.** 클리어한 스테이지 인덱스는 영속 저장되고,
  재방문해 목표를 만들어도 보상·클리어창이 발생하지 않는다(중복 보상 방지).
...
### 인게임 상단 표시
- Stage는 **점수·콤보를 집계/표시하지 않는다.** 최상단 중앙에 점수 대신 **`STAGE N`**(현재 스테이지)을
  표시한다([[50-art-ux/layout]]).
