...
function main() {
  ...
  const reason = [
    ...
    "The required flow is: log → reconcile docs → reflect into docs → work from docs (.claude/rules/docs-pipeline.md).",
    readDocs
      ? "You consulted docs but recorded nothing."
      : "Reconcile the latest instruction against docs/ now.",
    target,
    "Reflect with the docs-write skill (.claude/rules/docs-auto-reflect.md): update the page, refresh its section index (+ docs/index.md MOC if the page is new), append the nearest log.md, and bump the page frontmatter `updated:` date.",
