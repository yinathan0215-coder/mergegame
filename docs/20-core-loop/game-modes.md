...
- **기본 활성 = Infinite.**
- **시작(Play) 버튼 라벨은 선택 모드를 따른다:** Infinite = `Game Start`, Stage = `Stage N`
  (현재 스테이지 번호). 토글을 Stage로 옮기면 시작 텍스트가 `Stage 1` 형태로 바뀐다.
- Play를 누르면 선택 모드로 Pool In-Game에 진입한다(`PLAY_CLICKED` + 모드,
  [[screen-flow]]). 모드는 한 세션 동안 고정된다.

## Infinite 모드

- **시작 카운트 = 30** ([[../30-systems/launch-count]], 수치 [[../40-balancing/game-modes]]).
...
- **마지막 행성 보너스:** 마지막 행성(블랙홀, 11단계)끼리 합성하면 두 블랙홀이 사라지고
  **카운트 +20** 을 준다(Infinite 한정, [[../30-systems/merge-rules]] · ADR
  [[../30-systems/decisions/2026-06-28-blackhole-infinite-count]]).
- **종료 조건:** **카운트를 모두 소진한 뒤 보드의 모든 행성이 멈추면** 세션이 끝나고 결과창이
  뜬다. 결과창은 전환 효과와 함께 등장해 스코어가 1부터 최종 점수까지 카운트업되고, 최고 점수
  갱신이면 `NEW RECORD`, 하단에 **최대 콤보 횟수**를 표시한다. 탭하면 **Title로 복귀**한다
  (연출 정본 [[../50-art-ux/result-window]]).
...
- **클리어 조건:** 카운트 안에 **목표 행성을 합성으로 만들면** 성공이다. 성공 시 **코인 +300** 을
  주고 클리어 결과창이 전환 효과와 함께 뜬다. 결과창 하단에는 **[다음 스테이지]** 와
  **[돌아가기]**(→Title) 버튼이 있다.
- **실패 조건:** 카운트를 모두 소진했는데 목표를 못 만들면 실패다. **Fail 결과창**이 전환 효과와
  함께 뜨고, 닫으면 **게임 화면으로 복귀**한다(같은 스테이지 보드).
...
| 위치 | Infinite | Stage |
|---|---|---|
| **좌하단** | 남은 Count + 큐의 Next 미리보기 | 남은 Count + 큐의 Next 미리보기 |
| **우측 영역** | 행성 충전 버튼(회전 지구 + `Planet Charge`) | — |
| **우하단** | — | 목표 행성 정보(이름 + 회전하는 행성 이미지) |
...
| `PoolInGame(Stage)` | 목표 행성 생성 | `StageClear` | +300코인·전환 |
| `PoolInGame(Stage)` | `COUNT_DEPLETED` & 목표 미달 | `StageFail` | Fail 결과창(전환) |
| `Result` | tap | `Title` | — |
...
- [[../40-balancing/game-modes]] — 모드 수치(카운트 30·충전·보너스·보상)
